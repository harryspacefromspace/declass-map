...
<div id="map">
  ...
  <div id="ov-panel">
    ...
    <button class="ov-btn" data-ov="nuclear"><span class="ov-icon">☢️</span>Nuclear Power Plants<span class="ov-badge" id="badge-nuclear"></span></button>
    <button class="ov-btn" data-ov="naval"><span class="ov-icon">⚓</span>Naval Bases<span class="ov-badge" id="badge-naval"></span></button>
    <button class="ov-btn" data-ov="spaceport"><span class="ov-icon">🛰️</span>Spaceports / Launch<span class="ov-badge" id="badge-spaceport"></span></button>
    <button class="ov-btn" data-ov="testsite"><span class="ov-icon">💥</span>Nuclear Test Sites<span class="ov-badge" id="badge-testsite"></span></button>
  </div>
...
// Every overlay category the data file may carry. airbases/silos also have a
// curated Cold War list merged in; the rest come straight from overlays.geojson
// (OSM, plus the curated test sites baked in by fetch_overlays.py).
const OV_CATS   = ['airbases', 'silos', 'nuclear', 'naval', 'spaceport', 'testsite'];
const OV_COLORS = {{airbases:'#4d9fff', silos:'#ff4d4d', nuclear:'#4de08a',
                   naval:'#40c4d6', spaceport:'#ffd54d', testsite:'#e879f9'}};
...
function loadOverlayData() {{
  ...
  overlayPromise = fetch(OVERLAY_URL)
    ...
    .then(gj => {{
      const byKind = {{}};
      for (const k of OV_CATS) byKind[k] = [];
...
// Only airbases and silos have curated Cold War lists; the rest are OSM (or,
// for test sites, curated inside overlays.geojson) so they merge against [].
function curatedFor(key) {{
  return key === 'silos' ? CW_SILOS : key === 'airbases' ? CW_AIRBASES : [];
}}

function ovMarker(lat, lon, name, key) {{
  const c = OV_COLORS[key] || '#aaa';
...
async function toggleOverlay(key) {{
  ...
  try {{
    ...
    const points = mergeSites(curatedFor(key), data && data[key]);
...
function refreshOvBadges() {{
  for (const key of OV_CATS) {{
    ...
    badge.textContent = overlayCache
      ? mergeSites(curatedFor(key), overlayCache[key]).length.toLocaleString()
      : '·';
  }}
}}
